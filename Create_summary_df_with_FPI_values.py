...
# This script loads individual files with FPI values to crete a single 
  df with all the FPI values for different simualtions as columns 
...
#      Generate_Flood_Proximity_Index_for_a_list_of_simulations.py.py
...
results_path=os.path.join(OutPath_files,FPI_folder_name)
...
#list_of_simulations= r'00-list-of-files-to-generate-FPI-summary.xls'

list_of_simulations= r'00-list-of-files-to-generate-FPI-summary-for-SWIFT-paper.xls'
...
df_summary_FPI  = pd.DataFrame()

# here you add the first columns to summary df:

file_path_first_file =  df_file_list['folder_path_FPI_files'][0]
file_name_first_file =  df_file_list['Name_FPI_files'][0]


first_file= os.path.join(file_path_first_file,file_name_first_file)

df_FPI_first_sim=pd.read_excel(first_file)

df1= df_FPI_first_sim[['TSF_id']]
df2= df_FPI_first_sim[['FPI','TSF_id']]

df_summary_FPI= pd.merge(left=df1, right=df2, left_on='TSF_id', right_on='TSF_id')

# Here you rename the FPI column and assing the name of the simulation
df_summary_FPI = df_summary_FPI.rename(columns={'FPI': list_of_IDs[0] })
...
for x in range(1,number_of_simulations,1):
    ...
    file_path =  df_file_list['folder_path_FPI_files'][x]

    file_name = df_file_list['Name_FPI_files'][x]
    ...
    # Here you extract the FPI values 
    df_FPI_values = df[['FPI','TSF_id']]
    
    # here you add the last column to the summary E
    df_summary_FPI= pd.merge(left=df_summary_FPI, right=df_FPI_values, left_on='TSF_id', right_on='TSF_id')
    
    # Here you rename the column with FPI 
    df_summary_FPI = df_summary_FPI.rename(columns={'FPI': list_of_IDs[x] })
